<?php
/**
 * API endpoint to fetch video data and glosses for a sentence
 */
...
try {
    ...
    // Get parameters
    $sentenceId = isset($_GET['sentenceId']) ? intval($_GET['sentenceId']) : null;
    if (!$sentenceId && isset($_POST['sentenceId'])) {
        $sentenceId = intval($_POST['sentenceId']);
    }
    
    // Validate sentenceId parameter
    if (empty($sentenceId) || $sentenceId <= 0) {
        $response['success'] = false;
        $response['errors'][] = 'Valid sentenceId parameter is required';
        http_response_code(400);
    } else {
        // Log the request
        $logger->logRequest('getZinnenVideos', $sentenceId);
        
        // Get video data for the sentence
        $videoData = $sentenceService->getVideoDataForSentence($sentenceId);
        
        if ($videoData === null) {
            $response['success'] = false;
            $response['errors'][] = 'No video data found for sentence ID: ' . $sentenceId;
    ...
    if (IS_PROD) {
        ...
        $response['errors'][] = 'An error occurred while fetching video data. Please try again later.';
    } else {
    ...
    // Log the error
    if (isset($logger)) {
        $logger->logRequest('error', $sentenceId ?? '', 'error', $e->getMessage());
    }
}

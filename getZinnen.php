<?php
/**
 * API endpoint to fetch sentences by thema
 */
...
try {
    ...
    // Get parameters
    $thema = $_GET['thema'] ?? $_POST['thema'] ?? null;
    ...
    // Validate thema parameter
    if (empty($thema)) {
        $response['success'] = false;
        $response['errors'][] = 'Thema parameter is required';
        http_response_code(400);
    } else {
        // Log the request
        $logger->logRequest('getZinnen', $thema);
        
        // Get sentences by thema
        $sentences = $sentenceService->getSentencesByThema($thema, $limit, $offset);
        
        $response['success'] = true;
        $response['data']['sentences'] = $sentences;
        $response['data']['thema'] = $thema;
        $response['data']['count'] = count($sentences);
    ...
    // Log the error
    if (isset($logger)) {
        $logger->logRequest('error', $thema ?? '', 'error', $e->getMessage());
    }
}
